...
public class Chatbot_Activity extends AppCompatActivity {
    ...
    private static String TAG = "Chatbot_Activity"; //원래 메인 액티비티였음. 수정했다.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.chatbot_main_activity);
        ...
        inputMessage = findViewById(R.id.message_1);
        btnSend = findViewById(R.id.btn_send);
        btnRecord = findViewById(R.id.btn_record);
        String customFont = "montserrat_regular.ttf";
        ...
        if (permission != PackageManager.PERMISSION_GRANTED) {
            Log.i(TAG, "기록 권한이 거부됨");
            makeRequest();
        } else {
            Log.i(TAG, "녹화 권한이 이미 부여됨");
        }
        ...
        switch (requestCode) {
            ...
            case RECORD_REQUEST_CODE: {
                ...
                if (grantResults.length == 0
                    ...
                    Log.i(TAG, "사용자에 의해 권한이 거부됨");
                } else {
                    Log.i(TAG, "사용자가 사용 권한을 부여함");
                }
                return;
            }
            ...
            case MicrophoneHelper.REQUEST_PERMISSION: {
                if (grantResults.length > 0 && grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                    Toast.makeText(this, "오디오 녹음 권한 거부됨", Toast.LENGTH_SHORT).show();
                }
            }
        }
    ...
    private void sendMessage() {
        ...
        if (!this.initialRequest) {
            ...
            Toast.makeText(getApplicationContext(), "마이크를 탭하여 음성인식", Toast.LENGTH_LONG).show();
        }
            ...
            public void run() {
                try {
                    if (watsonAssistantSession == null) {
                        ServiceCall<SessionResponse> call = watsonAssistant.createSession(new CreateSessionOptions.Builder().assistantId("d0b9ff84-049f-4759-a85f-f20658e49afe").build());
    ...
    //Record a message via Watson Speech to Text 음성텍스트로 메세지 녹음
    private void recordMessage() {
        if (listening != true) {
            ...
            listening = true;
            Toast.makeText(Chatbot_Activity.this, "듣는중....중지하려면 클릭하십시오", Toast.LENGTH_LONG).show();

        } else {
            try {
                microphoneHelper.closeInputStream();
                listening = false;
                Toast.makeText(Chatbot_Activity.this, "듣기 중지됨....시작하려면 클릭하십시오.", Toast.LENGTH_LONG).show();
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private boolean checkInternetConnection() {
        ...
        if (isConnected) {
            ...
            Toast.makeText(this, " 사용 가능한 인터넷 연결 없음 ", Toast.LENGTH_LONG).show();
            return false;
        }
    ...
    private void showError(final Exception e) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(Chatbot_Activity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
                e.printStackTrace();
            }
        });
    }
    ...
    public void dodu_click (View view){
        Toast.makeText(getApplicationContext(),"이미 두두 챗봇 화면 입니다.", Toast.LENGTH_SHORT).show();
    }
